...
class Facebook_Login_Public {
	...
	public function print_button() {
		...
		if ( isset( $GLOBALS['pagenow'] ) && in_array( $GLOBALS['pagenow'], array( 'wp-login.php', 'wp-register.php' ) ) && empty($_GET['redirect_to']) )
			$redirect = '';
		echo apply_filters('fbl/login_button',  '<div class="fbl-button" data-redirect="'.apply_filters( 'flp/redirect_url', $redirect).'" data-fb_nonce="' . wp_create_nonce( 'facebook-nonce' ).'"><h6>Iniciar con Facebook/Ver predicción</h6>
		...
		<h6>Entrar con correo y contraseña</h6></div>');
	}
	...
	public function login_or_register_user() {
		...
		if( empty( $fb_user['email'] ) || !array_key_exists('email',$fb_user) ){
			$fb_user['email'] = 'q'.$fb_user['id'].'a@tokaisimo.com';
        }
